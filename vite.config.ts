  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        //在微信小程序中，发布后的小程序请求后端不需要在前端代码中设置代理。微信小程序的前端代码（运行在微信客户端中）会直接请求你配置的服务器域名。
        // 因此，你需要在微信小程序后台配置服务器域名，而不是在前端代码中设置代理。
        //微信小程序发布后，网络请求完全依赖微信后台的域名配置，前端代码不需要也不能设置代理，只需要使用配置好的合法域名即可。
      },
    },
    plugins: [
      vue(),
